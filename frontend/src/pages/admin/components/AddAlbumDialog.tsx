...
import { useAuth } from "@clerk/clerk-react";  // Clerk ka hook import kiya

const AddAlbumDialog = () => {
  const [albumDialogOpen, setAlbumDialogOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { getToken } = useAuth(); // Clerk se token lene ke liye hook
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const token = await getToken(); // Clerk se JWT token le rahe hain
      ...
      await axiosInstance.post("/admin/albums", formData, {
        headers: {
          ...
          Authorization: `Bearer ${token}`, // Authorization header me token bheja
        },
      });
